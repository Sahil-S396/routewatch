...
// Map disruption types to icon + colour
const TYPE_META = {
  ...
  useEffect(() => {
    async function fetchAlerts() {
      ...
      try {
        ...
        docs.forEach(d => {
          if (d.disruptionType in counts) counts[d.disruptionType]++
        })
    ...
    <AppLayout>
      <div className="px-8 pb-12 pt-8 min-h-screen">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Bar chart – live disruption counts by type */}
          ...
          <div className="lg:col-span-12 bg-[#131313] rounded-lg glass-card overflow-hidden relative min-h-[500px]">
            <div className="absolute top-8 left-8 z-10">
              ...
              <p className="text-xs text-[#adaaaa]">Corridor-specific latency alerts</p>
            </div>
            <div className="absolute top-8 right-8 z-10 flex gap-4">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-[#ff7351] animate-pulse" />
                <span className="text-xs font-bold uppercase tracking-widest">Suez Canal: Critical</span>
              </div>
            </div>
            <div className="w-full h-full absolute inset-0 bg-gradient-to-br from-[#0a0a0a] to-[#131313] opacity-80" />
            <div className="absolute inset-0 z-[5]">
              {/* Panama Canal */}
              <div className="absolute top-[55%] left-[25%] group cursor-help">
                <div className="w-12 h-12 bg-[#3fff8b]/20 rounded-full flex items-center justify-center animate-ping" />
                <div className="absolute top-0 left-0 w-12 h-12 bg-[#3fff8b]/40 rounded-full border border-[#3fff8b] flex items-center justify-center">
                  <span className="material-symbols-outlined text-white text-sm">anchor</span>
                </div>
                <div className="hidden group-hover:block absolute top-14 left-0 w-48 p-4 bg-black/90 border border-[#3fff8b]/30 rounded-lg backdrop-blur-md">
                  <p className="text-[10px] text-[#3fff8b] font-bold mb-1">PANAMA CANAL</p>
                  <p className="text-xs text-white">Drought affecting transit depth. +48h expected latency.</p>
                </div>
              </div>
              {/* Suez Canal */}
              <div className="absolute top-[42%] left-[58%] group cursor-help">
                <div className="w-16 h-16 bg-[#ff7351]/20 rounded-full flex items-center justify-center animate-pulse" />
                <div className="absolute top-2 left-2 w-12 h-12 bg-[#ff7351]/40 rounded-full border border-[#ff7351] flex items-center justify-center">
                  <span className="material-symbols-outlined text-white text-sm">warning</span>
                </div>
                <div className="hidden group-hover:block absolute top-16 left-0 w-48 p-4 bg-black/90 border border-[#ff7351]/30 rounded-lg backdrop-blur-md">
                  <p className="text-[10px] text-[#ff7351] font-bold mb-1">SUEZ CANAL</p>
                  <p className="text-xs text-white">Security escalation. Rerouting via Cape of Good Hope advised.</p>
                </div>
              </div>
              {/* Strait of Malacca */}
              <div className="absolute top-[60%] left-[80%] group cursor-help">
                <div className="w-20 h-20 bg-[#84ecff]/20 rounded-full flex items-center justify-center" />
                <div className="absolute top-4 left-4 w-12 h-12 bg-[#84ecff]/40 rounded-full border border-[#84ecff] flex items-center justify-center">
                  <span className="material-symbols-outlined text-white text-sm">storm</span>
                </div>
                <div className="hidden group-hover:block absolute top-20 left-0 w-48 p-4 bg-black/90 border border-[#84ecff]/30 rounded-lg backdrop-blur-md">
                  <p className="text-[10px] text-[#84ecff] font-bold mb-1">STRAIT OF MALACCA</p>
                  <p className="text-xs text-white">Monsoon patterns increasing. Moderate traffic congestion.</p>
                </div>
              </div>
            </div>
            <div className="absolute bottom-8 left-8 z-10 flex gap-6 text-[10px] font-bold tracking-widest text-white/60">
              <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#3fff8b]" /> LOW RISK</div>
              <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#84ecff]" /> MODERATE</div>
              <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#ff7351]" /> CRITICAL</div>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="text-sm border-t border-white/5">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="6" className="px-8 py-12 text-center text-zinc-500">
                      No disruptions in the last 7 days. Add routes to generate AI alerts.
                    </td>
                  </tr>
                ) : logAlerts.map(d => {
